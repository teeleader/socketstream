  ...
  use: function(nameOrStore, options) {
    ...
    if (nameOrStore === 'redis') {
      ...
      sessionStore.on('connect', function() {
        return console.log("[Redis-SessionStore] Connected...");
      });
      sessionStore.on('disconnect', function() {
        return console.error("[Redis-SessionStore] Disconnected/Error...");
      });
      return sessionStore.on('error', function(err) {
        return console.error("[Redis-SessionStore] Error... " + err);
      });
    } else {
      return sessionStore = nameOrStore;
    }
  },
  get: function() {
    return sessionStore;
