...
exports.store = {
  use: function(nameOrStore, options) {
    var RedisStore;
    return sessionStore = nameOrStore === 'redis' ? (RedisStore = require('connect-redis')(connect), new RedisStore(options), RedisStore.on('connect', function() {
      return console.log("[Redis-SessionStore] Connected...");
    }), RedisStore.on('disconnect', function() {
      return console.error("[Redis-SessionStore] Disconnected...");
    })) : nameOrStore;
  },
  get: function() {
    return sessionStore;
